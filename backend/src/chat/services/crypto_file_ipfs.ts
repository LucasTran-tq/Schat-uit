...
const privateKeyPath = ENCRYPTED_KEY_PATH + 'rsa_privateKeyIPFS.pem';
const publicKeyPath = ENCRYPTED_KEY_PATH + 'rsa_publicKeyIPFS.pem';
...
export class CryptoFileService {
  ...
  encryptRSA(toEncrypt, pubkeyPath = publicKeyPath) {
  ...
  decryptRSA(toDecrypt, privkeyPath = privateKeyPath) {
