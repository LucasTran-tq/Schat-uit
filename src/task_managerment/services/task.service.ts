...
import { CheckListDto, ItemDto, TaskDto, TaskParticipantDto } from "../dto/task.dto";
...
export class TaskService {
    ...
    public async createTask(taskDto: TaskDto, author: User): Promise<any> {
        const { task_category_id, check_lists, observers_id, participants_id, owner_id, ...task_el } = taskDto;
        const observers = [];
        const participants = [];
        
        const owner = await this.userRepository.findByID(owner_id);
        const taskCategory = await this.taskCategoryRepository.getOne({ _id: taskDto.task_category_id });
        const checkLists: CheckList[] = [];
        const itemsList: Item[] = [];
        const taskParticipantsList: TaskParticipant[] = [];
        //TODO: handle convert user_id to user 
        const checkListPromise = check_lists.map(async (checkList: CheckListDto, index: number): Promise<CheckList> => {
            const { items, ...check_list_el } = checkList;
            //Return Items promise
            const itemsPromise = items.map(async (item: ItemDto, index: number): Promise<Item> => {
                const { task_participants, ...item_el } = item;
                //Return Participants promise
                const taskParticipantsPromise = task_participants.map(async (taskParticipant: TaskParticipantDto, index: number): Promise<TaskParticipant> => {
                        const { user_id, ...task_participant_el } = taskParticipant;
                        const user = await this.userRepository.findByID(taskParticipant.user_id);
                        console.log(user);
                ...
                // console.log(taskParticipantsList);
        ...
        if(participants_id.length > 0){
            const participantsListPromise = participants_id.map(async participantId => await this.userRepository.findByID(participantId));
            for(let i = 0; i < participantsListPromise.length; i++) {
                participants.push(await participantsListPromise[i]);
            }
        }
        // console.log(participantsListPromise);

        if(observers_id.length > 0){
            const observersListPromise = observers_id.map(async observerId => await this.userRepository.findByID(observerId));
            for(let i = 0; i < observersListPromise.length; i++){
                observers.push(await observersListPromise[i]);
            }
        }
