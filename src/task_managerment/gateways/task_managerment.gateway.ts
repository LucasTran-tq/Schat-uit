...
export class TaskManagermentGateway extends AppGateway {
    ...
    public async handleAddTask(client: Socket, taskDto: TaskDto): Promise<Task> {
        try {
            ...
            return await this.handleGetTaskById(client, newTask._id.toString());
        }
        catch (err) {
    ...
    public async handleSearchByTaskName(client: Socket, textName: string): Promise<Task[]> {
        try {
            const user = await this.getUserClient(client);
            ...
            return await this.server.to(client.id).emti('error', {
                statusCode: 404,
                success: false,
                message: 'User do not exist'
            })
    ...
    public async handleGetTaskById(client: Socket, taskId: string): Promise<Task> {
        try {
