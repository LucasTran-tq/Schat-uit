...
export default class RTCPeerConnection extends defineCustomEventTarget(...PEER_CONNECTION_EVENTS) {
  ...
  _registerEvents() {
    this._subscriptions = [EventEmitter.addListener('peerConnectionOnRenegotiationNeeded', ev => {
      ...
      this.remoteDescription = new RTCSessionDescription(ev.sdp); // @ts-ignore

      this.dispatchEvent(new MediaStreamEvent('addstream', {
        stream
      }));
      ...
      this.dispatchEvent(new MediaStreamEvent('removestream', {
        stream
      }));
      ...
      if (this.iceGatheringState === 'complete') {
        this.localDescription = new RTCSessionDescription(ev.sdp); // @ts-ignore
